...
def sediment_flux(
    ...
    # The minDT floor above can inflate the last substep beyond the
    # remaining time (tEnd - tNow); re-cap so that tNow never overshoots
    # tEnd. Overshooting breaks the exact-equality checkpoint condition
    # in model.py under the Underworld coupling (outputs stop after time0).
    CFLtime = min(CFLtime, tEnd - tNow)
    if verbose:
