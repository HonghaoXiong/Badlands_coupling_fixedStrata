...
class strataMesh:
    ...
    📥 **Functionality**:
    Manages the stratigraphic grid, including initialization from restart files,
    moving the mesh, and handling erosion/deposition layers.

    Args:
        sdx (float): discretisation value [m]
        bbX (List[float] | numpy.ndarray): extent of stratal regular grid along X
        bbY (List[float] | numpy.ndarray): extent of stratal regular grid along Y
        layNb (int): total number of stratigraphic layers
        xyTIN (numpy.ndarray): numpy float-type array containing the coordinates for each nodes in the TIN (in m)
        folder (str): name of the output folder.
        h5file (str): first part of the hdf5 file name.
        poro0 (float): initial porosity.
        poroC (float): porosity evolution coefficient.
        cumdiff (numpy.ndarray | int): numpy array containing cumulative erosion/deposition from previous simulation.
        rfolder (Optional[str]): restart folder.
        rstep (int): restart step.
    """
    ...
    def __init__(
        ...
        poroC: float,
        cumdiff: Any = 0,
        rfolder: Optional[str] = None,
        rstep: int = 0,
    ) -> None:

        self.ids: Optional[numpy.ndarray] = None
        self.ptsNb: Optional[int] = None
        self.oldload: Optional[numpy.ndarray] = None
        self.tree: Optional[cKDTree] = None
        self.folder: str = folder
        self.h5file: str = f"{h5file}.time"
        self.step: int = 0
        self.upper: Optional[numpy.ndarray] = None
        self.lower: Optional[numpy.ndarray] = None
        self.poro0: float = poro0
        self.poroC: float = poroC
        self.xyTIN: numpy.ndarray = xyTIN

        # User defined parameter
        self.dx: float = sdx

        # Create stratal regular grid
        self.nx: int = int(round((bbX[1] - bbX[0]) / sdx - 0.5) + 1)
        self.ny: int = int(round((bbY[1] - bbY[0]) / sdx - 0.5) + 1)
        self.xgrid: numpy.ndarray = numpy.linspace(bbX[0], bbX[1], num=self.nx)
        self.ygrid: numpy.ndarray = numpy.linspace(bbY[0], bbY[1], num=self.ny)
        xi, yi = numpy.meshgrid(self.xgrid, self.ygrid)
        self.xyi: numpy.ndarray = numpy.dstack([xi.flatten(), yi.flatten()])[0]

        # Partition mesh
        self.buildPartition(bbX, bbY)
        self.ptsNb = len(self.ids) if self.ids is not None else 0

        if rstep > 0:
            restartncpus = 0
            if rfolder is not None and os.path.exists(rfolder):
                folder_path = f"{rfolder}/h5/"
                fileCPU = f"sed.time{rstep}.hdf5"
                restartncpus = len(glob.glob1(folder_path, fileCPU))
                if restartncpus == 0:
                    print(f"⚠️ WARNING: The requested time step {rstep} for the restart simulation cannot be found in the restart folder. Proceeding without loading strata restart data.")
                    rstep = 0
            else:
                print("⚠️ WARNING: The restart folder is missing or the given path is incorrect. Proceeding without loading strata restart data.")
                rstep = 0
            ...
            if rstep > 0:
                ...
                df = h5py.File(f"{rfolder}/h5/sed.time{rstep}.hdf5", "r")
        ...
        self.stratIn: numpy.ndarray = numpy.zeros([self.ptsNb], dtype=int)
        self.stratElev: numpy.ndarray = numpy.zeros([self.ptsNb, layNb])
        self.stratThick: numpy.ndarray = numpy.zeros([self.ptsNb, layNb])
        self.stratPoro: numpy.ndarray = numpy.zeros([self.ptsNb, layNb])
        self.stratDepth: numpy.ndarray = numpy.zeros([self.ptsNb, layNb])
    ...
    def update_TIN(self, xyTIN: numpy.ndarray) -> None:
        """
    ...
    def move_mesh(
        ...
        Args:
            dispX (numpy.ndarray): numpy float-type array containing X-displacement for each nodes in the stratal mesh
            dispY (numpy.ndarray): numpy float-type array containing Y-displacement for each nodes in the stratal mesh
            cumdiff (numpy.ndarray): numpy float-type array containing the cumulative erosion/deposition of the nodes in the TIN
            verbose (bool): when True, output additional debug information (default: False).
        """
        ...
        l0 = self.nx
        l1 = l0 + self.nx
        u1 = self.ptsNb - self.nx # type: ignore
        u0 = u1 - self.nx
        shape = (l1 - l0, self.step + 1)

        if verbose:
            print(f" - move stratal mesh {time.process_time() - walltime}")
        ...
        if verbose:
            print(
                f" - create deformed stratal mesh arrays {time.process_time() - walltime}"
            )
        ...
        if verbose:
            print(
                f" - create deformed stratal mesh kd-tree {time.process_time() - walltime}"
            )

        walltime = time.process_time()
        distances, indices = deformtree.query(self.xyi, k=4)
        if verbose:
            print(f" - query stratal mesh kd-tree {time.process_time() - walltime}")
        ...
        if verbose:
            print(
                f" - perform stratal mesh interpolation {time.process_time() - walltime}"
            )

        if verbose:
            print(f" - moving stratal mesh function {time.process_time() - st_time}")
    ...
    def buildStrata(
        ...
        Args:
            elev (numpy.ndarray): numpy float-type array containing the elevation of the nodes in the TIN
            cumdiff (numpy.ndarray): numpy float-type array containing the cumulative erosion/deposition of the nodes in the TIN
            sea (float): sea level elevation
            boundsPt (int): number of nodes on the edges of the TIN surface.
            write (int): flag for output generation
            outstep (int): step for output generation

        Returns:
            sub_poro (numpy.ndarray): numpy array containing the subsidence induced by porosity change.
        """

        selev = numpy.zeros(len(self.xyi))
        distances, indices = self.tree.query(self.xyi, k=self.searchpts) # type: ignore
        with numpy.errstate(divide="ignore"):
            weights = 1.0 / distances ** 2
        ...
        if self.ids is not None:
            ...
            return numpy.zeros(len(self.xyTIN[:, 0]))


    def buildPartition(self, bbX: Any, bbY: Any) -> None:
        """
        Define a partition for the stratal mesh.

        Args:
            bbX (Any): box boundary for X-axis
            bbY (Any): box boundary for Y-axis
        """
    ...
    def depoLayer(self, ids: numpy.ndarray, depo: numpy.ndarray) -> numpy.ndarray:
        """
        Add deposit to current stratigraphic layer.

        Args:
            ids (numpy.ndarray): index of points subject to deposition
            depo (numpy.ndarray): value of the deposition for the given point (m)

        Returns:
            subs (numpy.ndarray): numpy array containing the subsidence induced by porosity change.
        """
        ...
        subs = numpy.zeros(self.ptsNb) # type: ignore
        if self.poro0 > 0:
    ...
    def eroLayer(self, nids: numpy.ndarray, erosion: numpy.ndarray) -> None:
        """
        Erode top stratigraphic layers.

        Args:
            nids (numpy.ndarray): index of points subject to erosion
            erosion (numpy.ndarray): value of the erosion for the given points (m)
        """
    ...
    def layerMesh(self, topsurf: numpy.ndarray) -> None:
        """
        Define stratigraphic layers mesh.

        Args:
            topsurf (numpy.ndarray): elevation of the regular surface
        """
    ...
    def write_hdf5_stratal(self, outstep: int) -> None:
        """
        This function writes for each processor the HDF5 file containing sub-surface information.

        Args:
            outstep (int): output time step.
        """

        sh5file = f"{self.folder}/{self.h5file}{outstep}.hdf5"
